...
(ns cljm.repl
  ...
  (:require [clojure.string :as string]
            ...
            [cljm.compiler :as comp]
            [cljm.analyzer :as ana]
            [cljm.tagged-literals :as tags]
            [cljm.closure :as cljmc]))

(def ^:dynamic *cljm-verbose* false)
...
(defn load-namespace
  ...
  (let [sym (if (and (seq? sym)
        ...
        deps (->> (cljmc/add-dependencies opts {:requires [(name sym)] :type :seed})
  ...
  ([repl-env env filename form wrap]
     (try
       (let [ast (ana/analyze env form)
             ...
             wrap-js (comp/emit-str (binding [ana/*cljm-warn-on-undeclared* false
                                              ana/*cljm-warn-on-redef* false
                                              ana/*cljm-warn-on-dynamic* false
                                              ana/*cljm-warn-on-fn-var* false
                                              ana/*cljm-warn-fn-arity* false]
  ...
  (with-open [r (io/reader stream)]
    (let [env (ana/empty-env)
      ...
      (loop [r (read pbr false eof false)]
        (let [env (assoc env :ns (ana/get-namespace ana/*cljm-ns*))]
...
(defn- wrap-fn [form]
  (cond (and (seq? form) (= 'ns (first form))) identity
        ('#{*1 *2 *3} form) (fn [x] `(cljm.core.pr-str ~x))
        :else (fn [x] `(cljm.core.pr-str
                       (let [ret# ~x]
                         (do (set! *3 *2)
                             (set! *2 *1)
                             (set! *1 ret#)
...
(defn- eval-and-print [repl-env env form]
  (let [ret (evaluate-form repl-env
                           (assoc env :ns (ana/get-namespace ana/*cljm-ns*))
                           "<cljm repl>"
                           form
                           (wrap-fn form))]
...
(defn- read-next-form []
  (try {:status :success :form (binding [*ns* (create-ns ana/*cljm-ns*)
  ...
  (let [load-file-fn (fn [repl-env file] (load-file repl-env file))]
    {'in-ns (fn [_ quoted-ns]
              (let [ns-name (second quoted-ns)]
                ...
                (set! ana/*cljm-ns* ns-name)))
     'load-file load-file-fn
     'clojure.core/load-file load-file-fn
     'load-namespace (fn [repl-env ns] (load-namespace repl-env ns))}))

(defn repl
  "Note - repl will reload core.cljm every time, even if supplied old repl-env"
  [repl-env & {:keys [verbose warn-on-undeclared special-fns]}]
  (prn "Type: " :cljm/quit " to quit")
  (binding [ana/*cljm-ns* 'cljm.user
            *cljm-verbose* verbose
            ana/*cljm-warn-on-undeclared* warn-on-undeclared]
    (let [env {:context :statement :locals {}}
          special-fns (merge default-special-fns special-fns)
          is-special-fn? (set (keys special-fns))]
      (-setup repl-env)
      (loop []
        (print (str "ClojureScript:" ana/*cljm-ns* "> "))
        (flush)
